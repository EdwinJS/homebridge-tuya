...
class OilDiffuserAccessory extends BaseAccessory {
    ...
    _verifyCachedPlatformAccessory() {
        ...
        const {Service} = this.hap;const BaseAccessory = require('./BaseAccessory');
const async = require('async');
...
class OilDiffuserAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        humidifierService.getCharacteristic(Characteristic.CurrentRelativeHumidity)
            .updateValue(this.dpActive ? this._getRotationSpeed(dps) : 0)
    ...
    setActive(value, callback) {
        if (this.characteristicActive.value !== value){
            const {Characteristic} = this.hap;
            console.log('329', this.characteristicActive.value, value);
            switch (value) {
    ...
    // Will hardcode water levels until its needed -Ed
    ...
    setRotationSpeed(value, callback) {
        ...
        if (value === 0) {
            ...
            console.log('404', value, this._hkRotationSpeed, this.characteristicRotationSpeed.value);
            ...
            console.log('407', newSpeed, currentSpeed);
            if (this.enableFlipSpeedSlider) this._hkRotationSpeed = this.convertRotationSpeedFromTuyaToHomeKit(newSpeed);

            if (newSpeed !== currentSpeed) {
                console.log('different values when setting so it should set now - REALLY SHOULD CHANGE TESTESTESTESTESTESTEST')
    ...
    convertRotationSpeedFromTuyaToHomeKit(value) {
        console.log('current value th', value);
        if (this._isBelleLife()) {
    ...
    convertRotationSpeedFromHomeKitToTuya(value) {
            console.log('current value ht', value);
            if (this._isBelleLife()) {
                if (value < 2)
                    return this.cmdLow;
                else if (value < 3)
